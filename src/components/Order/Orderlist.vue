  ...
  <el-row class="warp" style="padding:20px 0 0 20px;">
    ...
    <el-col :span="24" class="warp-main">
      ...
      <el-form :inline="true">
        ...
        <el-form-item>
          <el-radio-group size="small" v-model="searchtype">
            <el-radio-button label="2">按商家</el-radio-button>
            <el-radio-button label="1">按地区</el-radio-button>
          </el-radio-group>
        </el-form-item>

        <el-form-item label="地区：" v-if="searchtype==1">
          <el-select v-model="filter.city" placeholder="全部地区" @change="changeadd" size="small">
            <el-option v-for="item in shopadd" :label="item.name" :value="item.id" :key="item.id"></el-option>
          </el-select>
        </el-form-item>

        <el-form-item v-if="searchtype==1">
          <el-input size="small" v-model="filter.address" placeholder="请输入区、县" style="min-width: 225px;" ></el-input>
        </el-form-item>

        <el-form-item label="商家：" v-if="searchtype==2">
          <el-select v-model="filter.shopid" placeholder="全部商家" @change="changeshop">
            <el-option v-for="item in shoplist" :label="item.name" :value="item.id" :key="item.id"></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="订单类型：">
          <el-select  size="small" v-model="filter.type" placeholder="全部订单" @change="changetype">
        ...
        <el-form-item label="订单状态：">
          <el-select v-model="filter.state" placeholder="全部订单" @change="changestate" size="small">
        ...
        <el-form-item>
          <el-input size="small" v-model="filter.search" placeholder="请输入订单号" style="min-width: 225px;" ></el-input>
        </el-form-item>
      ...
      <el-table :data="list" border stripe size="small" id="out-table" style="width:95%;">
        ...
        <el-table-column prop="score" label="积分" min-width="150" align="center">
        </el-table-column>
  ...
  <el-dialog title="订单详情" :visible.sync="dialogSeeVisible" width="1200px" center>
    <el-form label-position="right" label-width="110px" size="mini">
      ...
      <el-form-item label="总计：" class="fw6">
        <div class="fw4" v-if="currow.type=='scoreOrder'">{{currow.score}}积分</div>
        <div class="fw4" v-if="currow.type!=='scoreOrder'">￥{{currow.price}}</div>
      </el-form-item>
      ...
      <el-form-item label="联系方式：" class="fw6">
        <div class="fw4" v-if="currow.address">姓名： {{currow.address[0].name}}</div>
        <!-- <div class="fw4">地址： {{currow.address[0].address}}</div> -->
        <div class="fw4" v-if="currow.address">电话： {{currow.address[0].phone}}</div>
...
<!--           <el-table-column prop="detail" label="商品规格" min-width="100" align="center">
          ...
          <el-table-column prop="price" label="单价" min-width="100" align="center">
           <template slot-scope="scope">
            <p class="fw4" v-if="currow.type!=='scoreOrder'">￥{{scope.row.price}}</p>
            <p class="fw4" v-if="currow.type=='scoreOrder'">{{scope.row.score}}积分</p>
          </template>
        </el-table-column>
    ...
    <el-form-item label="商铺名称：" class="fw6">
      <template slot-scope="scope">
        <div class="fw4">{{currow.store.name}}</div>
      </template>
    </el-form-item>
...
<script>
  ...
  import { shopaddGet1 } from '../../api/api';
  ...
  export default {
   ...
   methods:{
    ...
    getshop(){
      var allParams = '?page='+ this.currentPage + '&limit=1000';
    ...
    getadd(){
      var allParams = ''
      shopaddGet1(allParams).then((res) => {
        console.log(res.data)
        this.shopadd=res.data;
    ...
    getlist(){
      var allParams = '?page='+ this.currentPage + '&limit=' + this.limit+ '&search=' + this.filter.search+'&start=' + this.filter.start+'&end=' + this.filter.end+'&state='+this.filter.state+'&type='+this.filter.type+'&city='+this.filter.city+'&address='+this.filter.address+'&shopid='+this.filter.shopid;

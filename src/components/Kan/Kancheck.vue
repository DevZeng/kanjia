...
<!--         <el-table-column prop="time" label="砍价限时" min-width="100" align="center">
        ...
        <el-table-column prop="state" label="状态" min-width="100" align="center">
          <template slot-scope="scope">
            <el-tag size="small" type="info" v-show="scope.row.state==1">待审核</el-tag>
            <el-tag size="small" type="success" v-show="scope.row.state==2">已通过</el-tag>
            <el-tag size="small" type="danger" v-show="scope.row.state==3">不通过</el-tag>
          </template>
        </el-table-column>

        <el-table-column label="操作" min-width="280" align="center">
         <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleEdit(scope.$index, scope.row)" v-show="scope.row.state">编辑</el-button>
          <el-button type="success" size="mini" @click="handlePass(scope.$index, scope.row)" v-show="scope.row.state" data-state="2">通过</el-button>
          <el-button type="info" size="mini" @click="handleReject(scope.$index, scope.row)" v-show="scope.row.state" data-state="3">不通过</el-button>

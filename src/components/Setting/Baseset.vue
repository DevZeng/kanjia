...
<!--               <el-radio-group v-model="signset.setype" size="small" class="mt5" v-show="show">
        ...
        <el-tab-pane label="倍数设置" name="times">

          <el-form ref="times" :model="times" label-width="160px" class="form" status-icon size="small" style="width:600px;" :rules="jfrule">

            <el-form-item label="砍价次数：">
              <el-input class="numinput" placeholder="请输入下限" v-model="times.kandown"></el-input>
              <el-input class="numinput" placeholder="请输入上限" v-model="times.kanup"></el-input>
            </el-form-item>

            <el-form-item label="浏览次数：">
              <el-input class="numinput" placeholder="请输入下限" v-model="times.lookdown"></el-input>
              <el-input class="numinput" placeholder="请输入上限" v-model="times.lookup"></el-input>
            </el-form-item>

            <el-form-item>
              <el-button size="small" type="primary" @click="posttime">提交</el-button>
            </el-form-item>
...
<script>
  ...
  import { timesPost } from '../../api/api';
  import { timesGet } from '../../api/api';
  ...
  export default {
    ...
    methods:{
      ...
      gettime(){
        var allParams = '';
        timesGet(allParams).then((res) => {
          console.log(res)
          this.times={
            kandown:res.data[0].detail,
            kanup:res.data[1].detail,
            lookdown:res.data[2].detail,
            lookup:res.data[3].detail
          }
        });
      },
      ...
      posttime(){
        var allParams={
          configs:[{
            title:'kandown',
            detail:this.times.kandown
          },
          {
           title:'kanup',
           detail:this.times.kanup
         },
         {
          title:'lookdown',
          detail:this.times.lookdown
        },
        {
         title:'lookup',
         detail:this.times.lookup
       }]
     }

     timesPost(allParams).then((res) => {
      this.$message({
        message: '提交成功',
        type: 'success'
      });

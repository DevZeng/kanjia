<template>
  <el-row class="warp" style="padding:20px 0 0 20px;">
    ...
    <el-col style="width: 1030px;">
      <el-form ref="newshop" :model="newshop" label-width="120px" :rules="rules" style="min-width: 1030px;" status-icon>
        ...
        <el-form-item label="商店地址:" prop="zuobiao">
          <el-input v-model="zuobiao" placeholder="当前坐标" style="width:300px;display: none;" id="poi_cur" size="small" ref="poi_cur"></el-input>

          <el-input placeholder="当前地址" style="width:300px;" id="addr_cur" size="small"></el-input>

          <input class="search_t" placeholder="请输入关键词搜索" onKeyPress="if(event.keyCode==13) {btnSearch.click();return false;}" style="width:300px;padding-left: 10px;" size="small" />

          <el-button type="primary" id="btn_search" size="small">搜索</el-button>

          <div style="position:relative;">

            <div id="tooles">
              <div id="cur_city">
                <strong>广州市</strong><span class="change_city">[<span style="text-decoration:underline;">更换城市</span>]</span><span class="change_city"><span id="level">当前缩放等级：10</span></span>
...
<script>
   ...
   methods:{
    ...
    save(){
      ...
      this.$refs.newshop.validate((valid) => {
        if (valid) {
          var addr_cur = document.getElementById("addr_cur");
          var item =this.$refs.poi_cur.value.split(',')

          var allParams={
            name:this.newshop.name,
            manager:this.newshop.manager,
            lat:item[0],
            lon:item[1],
  ...
  createmap(){
   ...
   var map = new qq.maps.Map(container, {zoom: 16}),
   ...
   qq.maps.event.addListener(map, "click", function (e) {
    ...
    that.zuobiao = e.latLng.getLat().toFixed(6) + "," + e.latLng.getLng().toFixed(6)
           // console.log(that.zuobiao)

           url3 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?location=" + e.latLng.getLat() + "," + e.latLng.getLng() + "&key=KLVBZ-UY4KU-TOHVG-2QSV5-LZSE6-KJFUD&output=jsonp&&callback=?");
   ...
   var isNoValue = false;
          ...
          qq.maps.event.addDomListener(btnSearch, 'click', function () {
            ...
            url = encodeURI("https://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region(" + query_city + ",0)&page_size=9&page_index=1&key=KLVBZ-UY4KU-TOHVG-2QSV5-LZSE6-KJFUD&output=jsonp&&callback=?");
            $.getJSON(url, function (result) {
              ...
              if (result.count) {
                ...
                each(result.data, function (n, ele) {
                  ...
                  var leftn = -54 - 17 * n;
                  order.style.cssText = "width:17px;height:17px;margin:3px 3px 0px 0px;float:left;background:url(../../../static/images/map/marker_n.png) " + leftn + "px 0px";
...
function setFlagClicked(arr, index) {
  each(markerArray, function (n, ele) {
    if (n == index) {
      ...
      that.zuobiao = latLng.getLat().toFixed(6) + "," + latLng.getLng().toFixed(6);
      console.log(that.zuobiao)
    } else {
      ele.isClicked = false;
...
city.onclick = function (e) {
  ...
  if (target.className.indexOf("city_name")== 7) {
    if(target.innerHTML.indexOf('<!---->')>-1){
      var str=target.innerHTML.split("<")[0]
    }else{
     var str=target.innerHTML
   }
   curCity.children[0].innerHTML = str;
   url2 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?region=" + curCity.children[0].innerHTML + "&address=" + curCity.children[0].innerHTML + "&key=KLVBZ-UY4KU-TOHVG-2QSV5-LZSE6-KJFUD&output=jsonp&&callback=?");
...
$(".search_t").autocomplete({
  source:function(request,response){
    url4 = encodeURI("https://apis.map.qq.com/ws/place/v1/suggestion/?keyword=" +  request.term + "&region=" + curCity.children[0].innerHTML + "&key=KLVBZ-UY4KU-TOHVG-2QSV5-LZSE6-KJFUD&output=jsonp&&callback=?");
